...
void TempleExpandPseudo::ExpandSETCC(MachineBasicBlock &MBB,
  ...
  case Temple::SETEQr:
    ...
    ADDiPseudo(Temple::T0, 12); // jump address calculation, need verification

    SUBrPseudo(LHS, RHS); // $ra-$rb
  ...
  case Temple::SETNEr:
    ...
    JLBuilder(Temple::T0, COND_NEVER, Temple::ZERO); // load PC
    ADDiPseudo(Temple::T0, 13); // jump address calculation, need verification
...
//   dbgs() << "TempleExpandPseudo:" << MI << "\n";
  ...
  case Temple::SRLr: {
    ...
    RegBuilder(MOVE, Temple::T3); // flag stash

    ADDrPseudo(Temple::T2, Temple::ALLONE);
    RegBuilder(MOVE, Temple::T2); // decrement shifter

    JLBuilder(Temple::ZERO, COND_ALWAYS, Temple::T0); // jump to loop
    // end:
    RegBuilder(NOR, Temple::ALLONE); // clear ACC
  ...
  case Temple::SRAr: {
    // T0:jump addr  T1:MSB stash  T2:shiftee  T3:FLAG stash
    RegBuilder(NOR, Temple::ALLONE);    // clear ACC
    RegBuilder(ADD, GetOperandReg(1));  // ACC = $rb(shiftee)
    RegBuilder(MOVE, GetOperandReg(0)); // copy to result reg
    ...
    // loop:
    JLBuilder(Temple::T0, COND_NEVER, Temple::ZERO);
    ADDiPseudo(Temple::T0, 27);
    RegBuilder(MOVE, Temple::T0);                // end label address calc
    JLBuilder(Temple::ZERO, COND_Z, Temple::T0); // jump to end
  ...
  case Temple::BLTU: {
    ...
    JLBuilder(Temple::T0, COND_NEVER, Temple::ZERO); // load program counter
    ADDiPseudo(Temple::T0, 15);                      // need verification
    RegBuilder(MOVE, Temple::T0);

    RegBuilder(NOR, Temple::ALLONE);
    RegBuilder(NOR, GetOperandReg(2));
    RegBuilder(ADD, Temple::ONE);
    JLBuilder(Temple::ZERO, COND_C, Temple::T0); // branch __C fallthrough
    RegBuilder(ADD, GetOperandReg(1));           // 4byte
  ...
  case Temple::BGEU: {
    ...
    RegBuilder(NOR, GetOperandReg(2));
    RegBuilder(ADD, Temple::ONE);
    JLBuilder(Temple::ZERO, COND_C, Temple::T1); // branch __C fallthrough
    RegBuilder(ADD, GetOperandReg(1));           // 4byte
